...
### Changed
- **重构强制除名机制 (`maintainer-role.md`)**：将原"两步走"流程拆分为两级处理，并明确分级边界与状态流转：
    - **7-A 即时除名**：适用于确认存在主观恶意或严重疏忽的极恶劣行为，无意识违规即使后果严重应优先走 7-B；被除名者可在 14 天内申诉，核心维护者团队 14 天内复核，超时未决议默认维持除名。
    - **7-B 裁决除名**：适用于严重但无主观恶意或损害尚可补救的违规，先降权并归档，7 天内表决；未通过则进入 14 天观察期（只读），期满由核心维护者多数表决决定是否恢复；超时兜底最长 30 天，延长期间可随时发起裁决，到期未裁决自动恢复为观察期。统一状态流转表与正文术语，明确"响应"和"表决"的定义。
- **权限矩阵新增观察期状态与状态-权限映射表 (`access-control.md`)**：增加 `Probation`（观察期）状态，并新增状态-权限映射表覆盖所有状态（正常、观察期、休眠、即时除名、永久除名、荣誉成员）的权限与退出方式。
