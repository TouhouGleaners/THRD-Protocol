...
### Changed
- **重构强制除名机制 (`maintainer-role.md`)**：将原"两步走"流程重组为"共同起点 + 分流"结构，退出/休眠与强制除名拆为独立章节：
    - **共同起点**：确认违规事实后立即降权并 24 小时内归档。
    - **7-A 即时除名**：适用于存在主观恶意或严重疏忽的行为，无需表决立即生效；14 天内可申诉复核，超时未决议默认维持除名，逾期不再受理。
    - **7-B 裁决除名**：适用于无主观恶意或损害尚可补救的违规，7 天内表决；未通过进入 14 天观察期（只读），期满多数表决决定恢复与否；超时兜底最长 30 天。
    - 新增时间窗口总览表（24h / 7d / 14d / 30d）。
