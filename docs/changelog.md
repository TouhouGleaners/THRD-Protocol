...
- **重构强制除名机制 (`maintainer-role.md`)**：将原"两步走"流程拆分为两级处理，并将退出/休眠与强制除名拆为独立章节：
    - **7-A 即时除名**：适用于确认存在主观恶意或严重疏忽的极恶劣行为；无意识违规即使后果严重应优先走 7-B；支持 14 天内申诉复核，超时未决议默认维持除名。
...
- **权限矩阵扩展 (`access-control.md`)**：新增观察期 (Probation)、即时除名 (Immediate Removal)、永久除名 (Permanent Removal) 状态定义，并新增状态-权限映射表覆盖全部六种状态。
