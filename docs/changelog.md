...
### Changed
- **重构强制除名机制 (`maintainer-role.md`)**：
    - 将原"两步走"流程拆分为 **C-1 即时除名**（适用于极恶劣行为，无需表决）与 **C-2 裁决除名**（适用于严重但未造成不可逆损害的违规，需表决）。
    - 裁决未通过时不再直接恢复全部权限，改为 14 天观察期（只读）。
    - 新增超时兜底条款：核心维护者均缺位时降权自动延长，最长不超过 30 天；到期未裁决自动恢复为观察期。
    - 明确即时隔离动作：收回写权限并在群聊中公告暂停身份。
    - 补充 C-1 申诉复核流程：14 天内完成复核，多数通过则维持除名，否则恢复权限。
