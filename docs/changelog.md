...
- **重构强制除名机制 (`maintainer-role.md`)**：将原"两步走"流程拆分为两级处理，并明确分级边界与状态流转：
    ...
    - **7-B 裁决除名**：适用于严重但无主观恶意或损害尚可补救的违规，先降权并归档，7 天内表决；未通过则进入 14 天观察期（只读），期满由核心维护者多数表决决定是否恢复；超时兜底最长 30 天，延长期间可随时发起裁决，到期未裁决自动恢复为观察期。统一状态流转表与正文术语。
- **权限矩阵新增观察期状态 (`access-control.md`)**：在权限级别说明中增加 `Probation`（观察期），对应 7-B 裁决未通过或超时兜底到期后的只读过渡状态。
