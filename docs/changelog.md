...
### Changed
- **重构强制除名机制 (`maintainer-role.md`)**：将原"两步走"流程拆分为两级处理，并将退出/休眠与强制除名拆为独立章节：
    - **7-A 即时除名**：适用于确认存在主观恶意或严重疏忽的极恶劣行为；无意识违规即使后果严重应优先走 7-B；支持 14 天内申诉复核，超时未决议默认维持除名；逾期不再受理同一 incident，新证据按新事件重新走流程。
    - **7-B 裁决除名**：适用于严重但无主观恶意或损害尚可补救的违规；先降权并归档，7 天内由核心维护者在归档 Issue 中表决；未通过进入 14 天观察期（只读），期满多数表决决定恢复与否；超时兜底最长 30 天，到期未裁决自动恢复为观察期。附状态流转表。
- **权限矩阵扩展 (`access-control.md`)**：新增观察期 (Probation)、即时除名 (Immediate Removal)、永久除名 (Permanent Removal) 状态定义，并新增状态-权限映射表覆盖全部六种状态，标注各终态的来源路径与退出方式。
