...
### Changed
- **重构强制除名机制 (`maintainer-role.md`)**：将原"两步走"流程拆分为两级处理：
    - **C-1 即时除名**：适用于极恶劣行为（篡改数据、泄露密钥、违法），确认事实后立即收回写权限并群聊公告，无需表决；被除名者可在 14 天内申诉，核心维护者团队 14 天内复核，多数通过则维持除名。
    - **C-2 裁决除名**：适用于严重但未造成不可逆损害的违规，先降权并归档，7 天内表决；未通过则进入 14 天观察期（只读），期满由核心维护者多数表决决定是否恢复；超时兜底最长 30 天，延长期间可随时发起裁决，到期未裁决自动恢复为观察期。
